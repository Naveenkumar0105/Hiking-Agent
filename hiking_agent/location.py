...
def get_current_location():
    ...
    Returns:
        tuple: A tuple containing the latitude, longitude, and state abbreviation.
               Returns (None, None, None) if the location cannot be determined.
    """
    try:
        # ip function gives the geocode for the location that i am currently in
        g = geocoder.ip('me')
        # ok tells if the lookup was successful
        if g.ok:
            state_abbrev = us_state_to_abbrev.get(g.state, g.state)
            # latlng[0]=latitude and latlng[1]=longitude
